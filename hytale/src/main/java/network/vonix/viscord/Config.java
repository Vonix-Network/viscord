 ...
 * Platform-agnostic implementation for Hytale.
 */
public class Config {
    ...
    public static void load() {
        // Get the current working directory and create config folder there
        Path workingDir = Path.of(System.getProperty("user.dir"));
        Path configDir = workingDir.resolve("config");

        Viscord.LOGGER.info("Working directory: {}", workingDir.toAbsolutePath());
        Viscord.LOGGER.info("Config directory: {}", configDir.toAbsolutePath());

        try {
            Files.createDirectories(configDir);
            Viscord.LOGGER.info("Config directory created/verified");
        } catch (IOException e) {
            Viscord.LOGGER.error("Failed to create config directory at {}", configDir.toAbsolutePath(), e);
        }
        configPath = configDir.resolve(CONFIG_FILE);
        Viscord.LOGGER.info("Config file path: {}", configPath.toAbsolutePath());
        ...
        if (Files.exists(configPath)) {
            try (Reader reader = new FileReader(configPath.toFile())) {
                ...
                Viscord.LOGGER.info("Loaded Viscord configuration from {}", configPath.toAbsolutePath());
            } catch (Exception e) {
            ...
            configData = new JsonObject();
            Viscord.LOGGER.info("Config file not found, creating default config at {}", configPath.toAbsolutePath());
        }

        // Save to ensure all keys exist with defaults
        save();
        Viscord.LOGGER.info("Config saved successfully");
    }
